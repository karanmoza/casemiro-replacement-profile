...
def generate_outputs(input_csv: str | Path | None = None) -> dict[str, Path]:
    ...
    shutil.copyfile(pdf_path, summary_pdf_path)
...
def find_chrome_executable() -> str:
    ...
    raise RuntimeError(
        "Chrome or Chromium is required to render the PDF from HTML. "
        "Install Chrome/Chromium or set CHROME_PATH to the executable."
    )


def render_html_to_pdf(html_path: str | Path, pdf_path: str | Path) -> Path:
    """Render the HTML report to PDF so the PDF matches the HTML source."""
